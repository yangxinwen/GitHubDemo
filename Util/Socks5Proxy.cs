...
namespace Util
{
    ...
    {
        ////测试软件(CCProxy、SuperProxy、Tight Socks5)
        private SocksProxy() { }
            ...
            string userName, string password)
        {
            ...
            {//Username/Password Authentication protocol
                nIndex = 0;
                request[nIndex++] = 0x01; // Version 5.  todo:0x05不行，改成0x01可通过，原因可能是代理服务未处理0x05这个值
            ...
            if (response[1] != 0x00)
                throw new ConnectionException(response[1] < errorMsgs.Length ? errorMsgs[response[1]] : "response unknown error from proxy server");
            // Success Connected...
            return s;
        }
    }
